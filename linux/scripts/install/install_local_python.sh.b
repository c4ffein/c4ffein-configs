...
SECURE_RANDOM_KEY=$(tr -dc a-f0-9 < /dev/urandom | dd bs=32 count=1 2> /dev/null)
NAME="Python"
VERSION="3.9.9"
...
TMP_STUFF="/tmp/"$SECURE_RANDOM_KEY
LOCAL_STUFF="$HOME/.local/external"

mkdir -p $LOCAL_STUFF
mkdir -p $TMP_STUFF

cat > $TMP_STUFF"/keys.asc" <<'endmsg'
...
wget "$TARBALL_LINK" -O $TMP_STUFF"/${TARBALL_NAME}"
wget "$TARBALL_ASC_LINK" -O $TMP_STUFF"/${TARBALL_NAME}.asc"
#gpg $TMP_STUFF"/"${TARBALL_NAME}".asc"

#echo -e $GPG_KEYS
#echo -e $GPG_KEYS > $TMP_STUFF"/keys.asc"
gpg --no-default-keyring --keyring $TMP_STUFF"/keyring.gpg" --import $TMP_STUFF"/keys.asc"

#gpg --recv-keys B26995E310250568  # Install key B26995E310250568: "Łukasz Langa (GPG langa.pl) <lukasz@langa.pl>"
#gpg --verify $TMP_STUFF"/"${TARBALL_NAME}".asc"
#echo $HASH
